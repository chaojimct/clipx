...
状态：**v0.10.4**（2026-09-20）。Windows 全功能日用，对齐并超越 WPF 1.9.8；另含弹窗质感（自绘滚动条 / 内高光 / 过渡动画）、检索体验（拼音命中高亮、空格分词交集、不完全拼音可高亮）、图上 OCR 选词、可选 OCR 精度拓展包（`--features ocr-rapid`）、文档型文件预览（docx/pptx/xlsx/pdf/文本）、应用内自动更新。Windows 上可关 `ClipboardX-filejump.exe`，剪贴板 + FileJump + Explorer 打字查找单进程，内存目标 10-30MB。安装包脚本 `scripts/clipx.iss`。
