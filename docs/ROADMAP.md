...
> 状态：v1.8 · 2026-09-21 · 当前阶段：**v0.10.7 已收尾，Windows 全功能日用**（对齐并超越 WPF 1.9.8 + WPF 历史首启自动导入 + 热键改键即时生效 + 弹窗质感 + 检索体验 + 图上 OCR 选词 + 文档预览 + 自动更新 + 右下角提示条反馈 + 更新下载进度 + 托盘瘦身与设置六页重排 + 浮层首帧残缺修复）。下一个迭代进入 **M6 macOS**。本机可关 `ClipboardX-filejump.exe`。
>
